...
public class ClienteNoDeseadoController {

    private final ClienteNoDeseadoService svc;
    private final UserRepository userRepo;

    public ClienteNoDeseadoController(ClienteNoDeseadoService svc, UserRepository userRepo) {
        this.svc = svc;
    ...
    private Long resolveUserId(Jwt jwt) {
        ...
        if (uid == null) uid = jwt.getClaim("uid");
    ...
    @PostMapping("/{id}/no-deseado/admin")
    public ResponseEntity<?> marcarDirecto(@AuthenticationPrincipal Jwt jwt, @PathVariable Long id) {
        if (jwt == null || !"ROLE_ADMIN".equals(jwt.getClaimAsString("role")))
            return ResponseEntity.status(403).build();
        try {
            svc.marcarDirecto(id);
    ...
    @PostMapping("/{id}/no-deseado/solicitar")
    public ResponseEntity<?> solicitar(@AuthenticationPrincipal Jwt jwt,
        ...
        if (usuarioNom == null || usuarioNom.isBlank()) usuarioNom = jwt.getSubject();
    ...
    @GetMapping("/{id}/no-deseado/pendiente")
    public ResponseEntity<Map<String, Boolean>> isPendiente(@PathVariable Long id) {
        return ResponseEntity.ok(Map.of("pendiente", svc.tieneSolicitudPendiente(id)));
    }

    @GetMapping("/no-deseado/pendientes")
    ...
    @PostMapping("/no-deseado/{id}/resolver")
    ...
    @GetMapping(value = "/no-deseado/confirm", produces = MediaType.TEXT_HTML_VALUE)
    public ResponseEntity<String> confirmarPage(@RequestParam String token) {
        com.washready.model.ClienteNoDeseadoSolicitud.Estado estado = svc.getEstado(token);
        ...
        if (estado != com.washready.model.ClienteNoDeseadoSolicitud.Estado.PENDIENTE) {
            String msgName = estado == com.washready.model.ClienteNoDeseadoSolicitud.Estado.APROBADA ? "Aprobada" : (estado == com.washready.model.ClienteNoDeseadoSolicitud.Estado.RECHAZADA ? "Rechazada" : "Expirada");
            String htmlR = """
        ...
        String html = """
            <!doctype html><html lang="es"><meta charset="utf-8">
            <title>Confirmar No deseado</title>
            <style>
            ...
            <body><div class="wrap">
              <h2>Confirmar cliente No deseado</h2>
              <div class="box">
                <p>Revisa la solicitud y confirma o rechaza la acción.</p>
                <form method="POST" action="/api/clientes/no-deseado/confirm">
                  <input type="hidden" name="token" value="%s">
                  <div class="actions">
                    <button class="btn btn-no" type="submit" name="accion" value="RECHAZAR">Rechazar</button>
                    <button class="btn btn-ok" type="submit" name="accion" value="APROBAR">Confirmar</button>
                  </div>
                </form>
              </div>
            </div></body></html>
    ...
    @PostMapping(value = "/no-deseado/confirm",
